...
public abstract class AbstractUpdateMapP<T, K, V> extends AsyncHazelcastWriterP {
    ...
    public static class SerializationContext<K> {
        ...
        SerializationContext(HazelcastInstance instance, IMap<K, ?> map) {
                ...
                partitionCount = partitionService.getPartitions().size();
                partitionIdFn = d -> partitionService.getPartition(d).getPartitionId();
                serializationService = ((SerializationServiceSupport)instance).getSerializationService();
                partitioningStrategy = (PartitioningStrategy<K>)((PartitionStrategyProvider) map).getPartitionStrategy();
        }
