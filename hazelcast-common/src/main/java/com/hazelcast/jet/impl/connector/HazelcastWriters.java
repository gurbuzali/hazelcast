...
public final class HazelcastWriters {
    ...
    public static <T, K, V> ProcessorMetaSupplier writeMapSupplier(
        ...
        return preferLocalParallelismOne(new WriteMapP.Supplier<>(name, toKeyFn, toValueFn));
    }
    ...
    public static <T, K, V> ProcessorMetaSupplier mergeMapSupplier(
        ...
        return updateMapSupplier(name, toKeyFn, (V oldValue, T item) -> {
    ...
    public static <T, K, V, R> ProcessorMetaSupplier updateMapSupplier(
        ...
        return ProcessorMetaSupplier.of(AbstractHazelcastConnectorSupplier.of(
    ...
    @Nonnull
    public static ProcessorMetaSupplier writeCacheSupplier(@Nonnull String name) {
        return ProcessorMetaSupplier.of(2, new WriteCachePSupplier<>(name));
    }

    @Nonnull
    public static ProcessorMetaSupplier writeListSupplier(@Nonnull String name) {
        return preferLocalParallelismOne(new WriteListPSupplier<>(name));
    }
    ...
    public static ProcessorMetaSupplier writeObservableSupplier(@Nonnull String name) {
        return new ProcessorMetaSupplier() {
            ...
            public Map<String, String> getTags() {
                return singletonMap(InternalObservable.OWNED_OBSERVABLE, name);
            }
    ...
    static RuntimeException handleInstanceNotActive(HazelcastInstanceNotActiveException e) {
        // if we are writing to a local instance, restarting the job should resolve the error
        return new RestartableException(e);
    }
    ...
    private static class WriteCachePSupplier<K, V> extends AbstractHazelcastConnectorSupplier {
        ...
        WriteCachePSupplier(@Nonnull String name) {
            this.name = name;
        }
        ...
        protected Processor createProcessor(HazelcastInstance instance, SerializationService serializationService) {
            ...
            ConsumerEx<ArrayMap<Data, Data>> bufferFlusher = buffer -> {
                try {
                    ...
                    throw handleInstanceNotActive(e);
                }
                buffer.clear();
            };
    ...
    private static class WriteListPSupplier<T> extends AbstractHazelcastConnectorSupplier {
        ...
        WriteListPSupplier( @Nonnull String name) {
            this.name = name;
        }
