...
public final class ServiceFactory<C, S> implements Serializable, Cloneable {
    ...
    private ServiceFactory(@Nonnull FunctionEx<? super Context, ? extends C> createContextFn) {
     ...
     *                        Context}. Called once per Jet member. It must be
     *                        stateless.
    ...
    public static <C> ServiceFactory<C, Void> withCreateContextFn(
            @Nonnull FunctionEx<? super Context, ? extends C> createContextFn
    ) {
     ...
     * become a part of the Jet job and available to {@link #createContextFn()}
     * as {@link Context#attachedDirectory
     ...
     */
    @Nonnull
    public FunctionEx<? super Context, ? extends C> createContextFn() {
     ...
     * #createContextFn()} as {@link Context#attachedFile
     * procSupplierContext.attachedFile(file.toString())} or
     * {@link Context#attachedDirectory
    ...
    protected ServiceFactory<C, S> clone() {
        try {
            ...
            throw new HazelcastException(getClass() + " is not cloneable", e);
        }
    }
}
