...
/**
 * The common aspect of {@code BatchStage batch} and {@link StreamStage
 ...
 * interface must be stateless and {@code Processor#isCooperative()
 * cooperative}.
 *
...
public interface GeneralStage<T> extends Stage {
    ...
    /**
     * Default value of max concurrent operations.
     * See {@link #mapUsingServiceAsync(ServiceFactory, int, boolean, BiFunctionEx)}.
     */
    public static final int DEFAULT_MAX_CONCURRENT_OPS = 4;

    /**
     * Default value for preserving the order.
     * See {@link #mapUsingServiceAsync(ServiceFactory, int, boolean, BiFunctionEx)}.
     */
    public static final boolean DEFAULT_PRESERVE_ORDER = true;

    /**
     * The number of in-flight batches being completed asynchronously.
     */
     ...
     * }</pre>
     *
     * @param mapFn a mapping function. It must be stateless and {@code
     ...
     *     {@code Processor#isCooperative() cooperative}.
     * @return the newly attached stage
     */
    @Nonnull
     ...
     *                  stateless and {@code Processor#isCooperative() cooperative}.
     ...
     * limited to {@value #MAX_CONCURRENT_ASYNC_BATCHES}
